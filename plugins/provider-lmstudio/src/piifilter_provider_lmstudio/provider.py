...
Connects to ``http://localhost:1234/v1`` and forwards filtered prompts
...
class LMStudioProvider(Provider):
    """Provider implementation for local LM Studio (OpenAI-compatible)."""
    ...
    async def forward(self, session: Session) -> str:
        ...
        try:
            ...
            msg = f"[PIIFilter Error: LM Studio returned {exc.response.status_code}]"
            logger.warning("LM Studio HTTP %s: %s", exc.response.status_code, exc.response.text[:200])
            return msg
        except httpx.RequestError as exc:
            msg = f"[PIIFilter Error: LM Studio request failed — {exc}]"
            logger.warning("LM Studio request error: %s", exc)
            return msg
    ...
    def _extract_text(self, data: dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.warning("Unexpected LM Studio response: %s", str(data)[:300])
            return "[PIIFilter Error: unexpected LM Studio response format]"