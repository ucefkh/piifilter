...
function getConfig() {
    ...
    return {
        ...
        mode: config.get<string>('mode', 'semantic'),
        enabled: config.get<boolean>('enabled', true),
    };
}

// ── PIIFilter API client ─────────────────────────────────────────────────────
    ...
    try {
        ...
        if (data.blocked) {
            vscode.window.showWarningMessage(
                `PIIFilter blocked: ${data.block_reason || 'Policy violation'}`
            );
...
function debounce(
    fn: (e: vscode.TextDocumentChangeEvent) => Promise<void>,
    delay: number
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration('piifilter')) updateStatus();
        })
    );

    // ── Register command ─────────────────────────────────────────────────────
    context.subscriptions.push(
        vscode.commands.registerCommand('piifilter.showStatus', () => {
            const { serverUrl, mode, enabled } = getConfig();
            vscode.window.showInformationMessage(
                `PIIFilter: ${enabled ? 'Enabled' : 'Disabled'}` +
                ` | Server: ${serverUrl}` +
                ` | Mode: ${mode}`
            );
        })
    ...
    const debouncedFilter = debounce(
        async (e: vscode.TextDocumentChangeEvent) => {
            ...
            if (!change || change.text.trim().length === 0) return;
    ...
    context.subscriptions.push(
        vscode.workspace.onDidChangeTextDocument(debouncedFilter)
    );
