#!/usr/bin/env python3
"""Generate adversarial PII dataset v4 — DeepSeek API, new seed, 10 new strategies.

Strategies: ROT13, base64, URL encoding, whitespace stretching, zero-width chars,
homoglyph substitution, mirrored brackets, slash encoding, character displacement,
white-on-white HTML.

Output: benchmarks/data/adversarial_v4.json (200+ examples)
"""
...
OBFUSCATION_STRATEGIES = [
    ...
    "base64 encoding (encode the PII value as base64 string)",
    "URL encoding (percent-encode special characters like @ -> %40, . -> %2E)",
    "whitespace stretching (insert multiple spaces/tabs between digits or chars)",
    "zero-width character insertion (insert U+200B zero-width spaces between characters)",
    "homoglyph substitution (replace Latin letters with visually similar Cyrillic: o→о, a→а, e→е, i→і)",
    "mirrored brackets (replace [] with reversed brackets ][, and () with )( oriented differently)",
    "slash encoding (replace . with / and @ with // or backslash patterns)",
    "character displacement (shift each character by +1 position in ASCII: a→b, b→c, 1→2)",
    "white-on-white HTML (embed PII inside <span style=\"color:white\"> or hidden HTML tags)",
]
...
def generate_batch(entity_type: str, strategy: str, count: int = 20) -> list[dict[str, Any]]:
    ...
    system_msg = (
        ...
        "be the deobfuscated (clean) version of the PII. "
    ...
    prompt = (
        ...
        f"All {count} examples must be distinct and use different people/companies."
    )
...
def main() -> None:
    print(f"╔═══ Adversarial PII Generator v4 ═══╗")
    ...
    # Build batch plan — each entity type gets 1 strategy
    ...
    for i, et in enumerate(ALL_ENTITY_TYPES):
        ...
        # Some get a second different strategy
        if i % 3 == 0:
            s2 = shuffled_strats[(i + 7) % len(shuffled_strats)]
    ...
    output = {
        "version": "4.0.0",
        "description": (
            f"Adversarial PII dataset v4 — DeepSeek, seed {RANDOM_SEED}, "
            f"{len(ALL_ENTITY_TYPES)} entity types, 10 new strategies."
        ),
        "seed": RANDOM_SEED,
        "model": MODEL,
    ...
    output_path = DATA_DIR / "adversarial_v4.json"
